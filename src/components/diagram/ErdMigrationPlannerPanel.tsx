        ...
        {plan.summary.total > 0 && (
          <section className="space-y-2 border-t border-border/60 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="grid grid-cols-3 gap-1 rounded-lg bg-muted/60 p-1">{(['postgresql', 'mysql', 'sqlserver'] as const).map(item => <button key={item} onClick={() => setDialect(item)} className={cn('rounded px-1 py-1 text-[8px] font-bold uppercase', dialect === item ? 'bg-background shadow-sm' : 'text-muted-foreground')}>{item === 'sqlserver' ? 'SQL Server' : item}</button>)}</div>
