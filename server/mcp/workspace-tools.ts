...
export function registerWorkspaceWriteTools(server: McpServer, userId: string) {
  server.registerTool("erd_patch_propose", {
    description: "Prepare granular ERD changes without resending or overwriting the full diagram. Supports add/update/delete for tables, columns, indexes, constraints, and relationships. IDs for new objects are generated by the server. Returns an exact preview plus ordered PostgreSQL/MySQL forward and rollback SQL; never writes data, so show the preview and obtain explicit confirmation before applying.",
    inputSchema: {
      uid: z.string().min(1).max(100),
      expected_version: z.number().int().min(0).optional(),
